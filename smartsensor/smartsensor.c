...
void smartsensor(void){
	...
	case 0x01:{
		ADC_read(0, 0, &SL0, 0);		// only save value to variable SL0
		Kirim = SL0;
	}break;

	/* Send high byte sensor 0	*/
	case 0x02:{
		ADC_read(0, &SH0, 0, 0);		// only save value to variable SL0
		Kirim = SH0;
	}break;

	/*	Send low byte sensor 1	*/
	case 0x03:{
		ADC_read(1, 0, &SL1, 0);		// only save value to variable SL0
		Kirim = SL1;
	}break;

	/*	Send high byte sensor 1	*/
	case 0x04:{
		ADC_read(1, &SH1, 0, 0);		// only save value to variable SL0
		Kirim = SH1;
	}break;

	/* Send low byte sensor 2	*/
	case 0x05:{
		ADC_read(2, 0, &SL2, 0);		// only save value to variable SL0
		Kirim = SL2;
	}break;

	/* Send low high byte sensor 2	*/
	case 0x06:{
		ADC_read(2, &SH2, 0, 0);		// only save value to variable SL0
		Kirim = SH2;
	}break;
